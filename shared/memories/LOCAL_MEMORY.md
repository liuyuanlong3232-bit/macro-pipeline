...
§
项目已清理：69个调试文件删除，本地+VPS+Git三方同步(commit f27af28)。根目录剩22个py(含4个旧周报备用)，scripts 19个。硬性规则：代码只能本地改→VPS测试→推Git。VPS cron用UTC时区(00:00=08:00 CST)。
§
...
§
VPS Hermes已停(6/16)。原因：用户觉得复杂没用。保留：hermes.db(23MB)+cron(17条)+scripts(19个)+数据采集。去掉：AI对话+QQ Bot。邮件发送不依赖Hermes(smtplib标准库)。